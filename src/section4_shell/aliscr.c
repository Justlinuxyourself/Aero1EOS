...
void cmd_run_script_file(char* args) {
    if (!args || *args == '\0') {
        vga_write("Usage: run <filename>\n");
        return;
    }

    // 1. Open the saved notebook file via POSIX layer
    int fd = posix_open(args);
    if (fd < 0) {
        vga_write("Error: Could not open script file.\n");
        return;
    }

    vga_write("[!] Stream Opened. Executing AliScript File: ");
    vga_write(args);
    vga_write("\n");

    char line_buffer[LINE_SIZE];
    int line_index = 0;
    sp = 0; // Clear stack pointer before execution

    // 2. Read characters line-by-line out of the stream
    while (1) {
        char c;
        int bytes_read = posix_read(fd, &c, 1);
        
        if (bytes_read <= 0) {
            // End of File reached! Process trailing commands if buffer has data
            if (line_index > 0) {
                line_buffer[line_index] = '\0';
                execute_aliscript_line(line_buffer);
            }
            break; 
        }
        ...
        if (c == '\n') {
            ...
            line_index = 0; // Reset buffer for next row
        } else {
