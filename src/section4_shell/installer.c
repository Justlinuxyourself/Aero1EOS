...
void cmd_install_os() {
    ...
    // 1. Write GRUB Stage 1 (Sector 0)
    vga_write("Writing GRUB MBR (Sector 0)... ");
    ide_write_sector_bytes(0, boot_img);
    vga_write("DONE\n");
    ...
    // We start at 0x1000 because that's where the Multiboot header is physically mapped
    ...
    vga_set_color(0x0A);
    vga_write("\nSUCCESS! AliOS is now installed on the hard drive.\n");
